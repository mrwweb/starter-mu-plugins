 ...
 *
 * @param stringl $html the HTML output of the view
 *
 * @return string the HTML output of the view with <section> replaced with <div>
 */
 ...
 *
 * @return string organizer title/name
 */
 ...
 *
 * @param string $link the HTML link to the venue
 * @param int    $venue_id the ID of the venue
 * @param bool   $full_link whether to return HTML markup (true) or URL (false)
 * @param string $url the URL of the venue
 *
 * @return string the modified HTML link to the venue
 */
 ...
 *
 * @param array $args the arguments for the tribe_events post type
 */
...
add_filter( 'tribe_template_html', __NAMESPACE__ . '\prevent_ajax_page_loads' );
/**
 ...
 *
 * @param string $html the HTML output of the view
 *
 * @return string the modified HTML output of the view
 */
function prevent_ajax_page_loads( $html ) {
 ...
 *
 * @return string the modified title of the archive
 */
function archive_title( $title ) {
	if ( 'tribe_events' === get_post_type() ) {

		if ( \tribe_is_past() || ( $_GET['eventDisplay'] ?? '' ) === 'past' ) { // phpcs:ignore I'm not actually using eventDisplay so we should be safe here
			$title = 'Past Events';
 ...
 */
define( 'TRIBE_DISABLE_TOOLBAR_ITEMS', true ); //phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedConstantFound
